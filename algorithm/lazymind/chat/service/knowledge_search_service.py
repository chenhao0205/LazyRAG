...
def search(user_id: str, query: str, kb_ids: List[str], top_k: int) -> List[KnowledgeSearchHit]:
    ...
    try:
        ...
        top_k = 10
    top_k = min(50, top_k if top_k > 0 else 10)
    try:
        retrievers, reranker, image_retriever = _ensure_kb_search_runtime()
        raw = search_kb({'query': query, 'filters': {'kb_id': kb_ids}, 'user_id': user_id}, retrievers=retrievers,
                        reranker=reranker, image_retriever=image_retriever, retriever_topk=max(20, top_k),
                        rerank_topk=max(20, top_k), k_max=top_k, image_topk=0)
...
def _hits_from_serialized(serialized: Any, allowed_kb_ids: List[str], limit: int) -> List[KnowledgeSearchHit]:
    ...
    if not isinstance(items, list):
        return []
    allowed, hits, seen = set(allowed_kb_ids), [], set()
    for item in items:
        if len(hits) >= limit or not isinstance(item, dict) or _is_image_only(item):
            continue
        kb_id, text = _string(item.get('kb_id')), _string(item.get('text'))
        if kb_id not in allowed or not text:
            continue
        ...
        if key in seen:
            continue
        seen.add(key)
        hits.append(KnowledgeSearchHit(kb_id=kb_id, doc_id=doc_id, chunk_id=chunk_id, text=text,
                                        score=_float(item.get('score')), title=_string(item.get('title') or item.get('file_name')),
...
def _is_image_only(item: dict) -> bool:
    ...
    return group == 'image' or bool(item.get('image_markdown') or item.get('local_path')) or (
        isinstance(metadata, dict) and (_string(metadata.get('type') or metadata.get('node_type')).lower() == 'image' or metadata.get('images')))


def _safe_source_url(value: Any) -> str:
    url = _string(value)
    parsed = urlparse(url) if url else None
    return url if parsed and parsed.scheme in ('http', 'https') and parsed.netloc else ''
