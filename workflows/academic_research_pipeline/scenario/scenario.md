...
1. **研究问题与方法设计**：生成有范围边界的 RQ Brief 和方法论蓝图，未知数据与审批状态保持未知。
2. **学术与知识库检索**：优先使用 LazyMind `academic_search`（由运行时选择 Sciverse 或其他可用学术源），同时检索当前会话已选知识库。两者均不可用时如实记录并继续，禁止虚构引用。
3. **文献核验与证据综合**：建立 `SRC-NNN/KB-NNN` 闭合证据池、注释书目、矛盾证据和研究空白，并注入共享 Writer 上下文。
4. **论文大纲生成**：复用 LazyMind Writer 生成或修改大纲，确定性检查层级、篇幅分配和证据映射；生成完成后暂停，用户可编辑和 AI 润色。
5. **论文全文生成**：Writer 流式逐章写作，保持可编辑初稿；外部主张只能引用已注册证据，生成完成后再次暂停。
6. **审稿前完整性检查**：确定性检查结构、篇幅和证据 ID，模型进行范围有界的主张—来源、数据、方法和七类 AI 研究失败模式审查；不声称自动完成抄袭检测。
7. **五视角同行评审**：Journal-Fit、方法、领域、跨学科和 Devil's Advocate 五种角色分别报告，再形成编辑决定与完整修订路线图。
8. **一轮修订审批**：基于路线图使用 Writer 定向修订，允许作者有依据地不同意评审意见。
9. **修订验证复审**：逐项比对原稿、路线图、作者响应和修订稿；仅输出 Accept、Minor 或 Major。
10. **可选二轮修订**：仅 Major 路由进入，修订后直接进入最终完整性，不再循环复审。
11. **最终完整性检查**：对最新稿件从新输入重跑，不沿用初检结论，并设置人工确认点。
12. **论文格式化与交付**：输出完整 Markdown；按启动参数输出 Markdown 或真实 DOCX。
13. **过程记录**：记录检索、审批、完整性、评审、修订和交付的可审计历史。
