...
func New(rt *runtime.Runtime, identity IdentityProvider, options Options) (*Server, error) {
	...
	if err := registry.Register(knowledgeListTool()); err != nil {
		return nil, err
	}
	if err := registry.Register(knowledgeGetTool()); err != nil {
		return nil, err
	}
	if err := registry.Register(knowledgeDocumentGetTool()); err != nil {
		return nil, err
	}
