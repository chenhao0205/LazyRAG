...
func TestWriteUserPreferenceDraftUpdatesSubmittedFrontmatter(t *testing.T) {
	db := newResourceFSTestDB(t)
	store.Init(db.DB, nil, nil)
	service := NewService(ServiceDeps{DB: db.DB})
	ref := ResourceRef{UserID: "u1", ResourceType: ResourceTypeUserPreference}
	...
	parsed, err := preferencefile.ParseFileContent(draft.Content)
	if err != nil {
		t.Fatalf("ParseFileContent returned error: %v", err)
	}
	if parsed.AgentPersona != "新助手" ||
		parsed.PreferredName != "新称呼" ||
		parsed.ResponseStyle != "新风格" {
		t.Fatalf(
			"draft frontmatter = (%q, %q, %q), want submitted values",
			parsed.AgentPersona,
			parsed.PreferredName,
			parsed.ResponseStyle,
		)
	}
	if parsed.Content != "新正文\n" {
		t.Fatalf("draft body = %q, want %q", parsed.Content, "新正文\n")
	}
}

func TestWriteUserPreferenceDraftUsesFirstSubmittedFrontmatterAndStripsRepeats(t *testing.T) {
	db := newResourceFSTestDB(t)
	store.Init(db.DB, nil, nil)
	service := NewService(ServiceDeps{DB: db.DB})
	ref := ResourceRef{UserID: "u1", ResourceType: ResourceTypeUserPreference}
	...
	if !strings.Contains(draft.Content, `agent_persona: 新助手`) ||
		...
		t.Fatalf("draft content did not use submitted frontmatter: %q", draft.Content)
	}
	if !strings.HasSuffix(draft.Content, "\n\n第一行\n新值\n") {
		t.Fatalf("draft content body = %q", draft.Content)
	}
	...
	changed := strings.Join(changedLines, "\n")
	for _, want := range []string{"agent_persona: 旧助手", "agent_persona: 新助手", "旧值", "新值"} {
		if !strings.Contains(changed, want) {
			t.Fatalf("changed diff lines = %q, want line containing %q", changed, want)
		}
	}
}
