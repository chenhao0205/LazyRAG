...
func TestCloudDocumentAdapterListSourcesRequestAndMapping(t *testing.T) {
	...
	if len(result.Sources) != 1 || result.Sources[0].ID != "source-1" || result.Sources[0].DocumentCount == nil || *result.Sources[0].DocumentCount != 7 {
...
func TestCloudDocumentAdapterListDocumentsRequestAndMapping(t *testing.T) {
	...
	if len(result.Documents) != 1 || result.Documents[0].ID != "doc-1" || result.Documents[0].ParseStatus != "PARSED" {
