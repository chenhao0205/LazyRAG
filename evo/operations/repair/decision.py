...
class DecisionModelAdapter(DecisionAgent):
    ...
    def decide(self, view: RepairView) -> RepairAction:
        prompt = (
            ...
            'You analyze the RAG failure and design experiments; never output source code or whole-file edits. '
            'code inspect searches and reads code; code edit_work writes or revises the Demo under work/. '
            'code edit_source asks OpenCode to make a minimal scoped source change after evidence supports it. '
            'workspace lists/reads/diffs and may write experiment input data '
            'under work/ only; shell runs a Demo command under work/; '
            ...
            'Code arguments: {operation: inspect|edit_work|edit_source, instruction: string}.\n'
            'Shell runs only {command: [python, demo/run_demo.py, --input, inputs/<name>.json], '
            'cwd: work, timeout_seconds?: int}; stdout must be exactly one JSON object.\n'
            'Test arguments: {level: L0|L1|L2}.\n'
            'Finish is accepted only when the current source hash has successful L0, L1 and L2 evidence. '
            'Any later edit_source changes the hash and makes older test evidence stale.\n'
...
__all__ = ['DecisionAgent', 'DecisionModelAdapter', 'ModelCall']
